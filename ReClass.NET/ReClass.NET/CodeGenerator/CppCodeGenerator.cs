...
namespace ReClassNET.CodeGenerator
{
	...
	public abstract class CustomCppCodeGenerator
	{
		...
		/// </summary>
		public virtual void OnGenerationBegin() { }
		...
		/// <summary>
		/// Returns <c>true</c> if the code generator can handle the given class.
		/// </summary>
		/// <param name="class">The class to check.</param>
		/// <returns>True if the code generator can handle the given class, false otherwise.</returns>
		public virtual bool CanHandleClass(ClassNode @class) 
		{ 
			return false; 
		}

		/// <summary>
		/// Outputs the code for the given class to the <see cref="TextWriter"/> instance.
		/// </summary>
		...
		public virtual void WriteClass(IndentedTextWriter writer, ClassNode @class, IEnumerable<ClassNode> classes, ILogger logger)
		{

		}

		/// <summary>
		/// Outputs the code for the node to the <see cref="TextWriter"/> instance.
		/// </summary>
		/// <param name="writer">The writer to output to.</param>
	...
	public class CppCodeGenerator : ICodeGenerator
	{
		...
		private static CustomCppCodeGenerator GetCustomCodeGeneratorForClass(ClassNode @class)
		{
			return customGenerators.FirstOrDefault(g => g.CanHandleClass(@class));
		}
		...
		public string GenerateCode(IReadOnlyList<ClassNode> classes, IReadOnlyList<EnumDescription> enums, ILogger logger)
		{
			using (var sw = new StringWriter())
			{
				using (var iw = new IndentedTextWriter(sw, "\t"))
				{
					...
					customGenerators.ForEach(g => g.OnGenerationBegin());
					...
					customGenerators.ForEach(g => g.OnGenerationEnd());
				}
		...
		private void WriteClass(IndentedTextWriter writer, ClassNode @class, IEnumerable<ClassNode> classes, ILogger logger)
		{
			...
			var custom = GetCustomCodeGeneratorForClass(@class);
			if (custom != null)
			{
				custom.WriteClass(writer, @class, classes, logger);
				writer.Indent++;
				WriteNodes(writer, @class.Nodes, logger);
				writer.Indent--;
				writer.WriteLine();
				return;
			}
